/* ==================== pastries.js – FINAL UPGRADED VERSION (2025) – BACKEND WISHLIST + TOASTIFY ==================== */
document.addEventListener("DOMContentLoaded", function () {
    ...
    class PastriesApiService {
        ...
        isCacheValid() {
            return this.cache.pastries && this.cache.lastFetch && (Date.now() - this.cache.lastFetch) < this.cache.cacheDuration;
        }
        ...
        mapApiToPastry(apiProduct) {
            ...
            return {
                ...
                name: apiProduct.productName || 'Pastry',
                category: 'Pastries',
                ...
                productQuantity: apiProduct.productQuantity || 0,
                productDiscount: apiProduct.productDiscount || ''
            };
        }
    ...
    /* ------------------------------------------------------------------ */
    function getDynamicBadge(pastry) {
        if (pastry.productQuantity === 0) {
            return { text: 'Out of Stock', color: 'bg-gray-600' };
        }

        if (pastry.originalPrice && pastry.originalPrice > pastry.price) {
            const discount = Math.round(((pastry.originalPrice - pastry.price) / pastry.originalPrice) * 100);
            return { text: `${discount}% OFF`, color: 'bg-red-600' };
        }

        if (pastry.productDiscount && pastry.productDiscount.trim() !== '') {
            return { text: pastry.productDiscount, color: 'bg-teal-700' };
        }
    ...
    /* ------------------------------------------------------------------ */
    /* 4. TOASTIFY NOTIFICATION (CENTERED & BEAUTIFUL) */
    /* ------------------------------------------------------------------ */
    ...
    /* ------------------------------------------------------------------ */
    /* 5. PASTRIES PAGE MANAGER – BACKEND WISHLIST READY */
    /* ------------------------------------------------------------------ */
    class PastriesPageManager {
        ...
        async init() {
            ...
            this.markWishlistedItemsOnLoad(); // Heart red on login
        }
        ...
        renderPastriesGrid(pastries) {
            ...
            productsGrid.innerHTML = pastries.map(pastry => {
                const badge = getDynamicBadge(pastry);
                const isOutOfStock = pastry.productQuantity === 0;

                return `
                    <div class="bg-white rounded-lg shadow-card overflow-hidden hover:shadow-lg transition-shadow duration-300 ${isOutOfStock ? 'opacity-70' : 'cursor-pointer'}" 
                         data-id="${pastry.id}" 
                         ${!isOutOfStock ? `onclick="window.location.href='/product-details.html?id=${pastry.id}'"` : ''}>
                        ...
                        <div class="relative">
                            ...
                            <!-- DYNAMIC BADGE -->
                            ${badge ? `
                                <div class="absolute top-2 left-2 ${badge.color} text-white px-3 py-1 text-xs font-bold rounded shadow">
                                    ${badge.text}
                                </div>` : ''}

                            <!-- WISHLIST BUTTON -->
                            <button class="wishlist-btn absolute top-2 right-2 bg-white p-2 rounded-full shadow-md z-10" 
                                    data-id="${pastry.id}"
                                    data-type="PRODUCT"
                                    onclick="event.stopPropagation(); pastriesPageManager.toggleWishlist(this)">
                        ...
                        <div class="p-4">
                            ...
                            <div class="flex justify-between items-center">
                                ...
                                <button class="${isOutOfStock ? 'bg-gray-400 cursor-not-allowed' : 'bg-primary hover:bg-secondary'} text-white px-4 py-2 rounded-lg transition-colors text-sm font-medium"
                                        ${isOutOfStock ? 'disabled' : `onclick="event.stopPropagation(); window.location.href='/product-details.html?id=${pastry.id}'"`}>
            ...
            this.markWishlistedItemsOnLoad(); // Re-apply heart status after render
        }
        ...
        async toggleWishlist(btn) {
            ...
            try {
                if (isActive) {
                    ...
                    if (wishlistItemId) {
                        await window.apiService.removeFromWishlist(wishlistItemId,'PRODUCT');
                    ...
                    await window.apiService.addToWishlist(productId, 'PRODUCT');
        ...
        async markWishlistedItemsOnLoad() {
            if (!window.apiService?.getUserId()) return;
            try {
                const wishlist = await window.apiService.getWishlist();
                wishlist.forEach(item => {
                    const btn = document.querySelector(`.wishlist-btn[data-id="${item.productId}"]`);
                    if (btn) {
                ...
                console.warn('Could not mark wishlist items on pastries page (CORS safe)', err);
            }
        }
    ...
    /* ------------------------------------------------------------------ */
    /* 6. LOGIN / PROFILE DROPDOWN + LOGOUT MODAL */
    /* ------------------------------------------------------------------ */
    function isLoggedIn() {
